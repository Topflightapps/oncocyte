...
class Vc_Frontend_Editor {
			...
			if ( isset( $shortcode['id'] ) && isset( $shortcode['string'] ) ) {
				if ( isset( $shortcode['tag'] ) ) {
					...
					if ( is_object( $shortcode_obj ) ) {
						...
						// START UNCODE EDIT
						// $output .= '</div>';
						$tags_array = apply_filters( 'uncode_js_composer_renderShortcodes_tags_array', array() );
						if ( in_array( $shortcode['tag'], $tags_array )) {
							$output .= '</div>';
						}
						// END UNCODE EDIT
					}
	...
	public function toString( $shortcode, $content ) {
		...
		$_div = '';
		$tags_array = apply_filters( 'uncode_js_composer_toString_tags_array', array() );
		if ( in_array( $shortcode['tag'], $tags_array )) {
			$_div = '</div>';
		}
		$break_array = apply_filters( 'uncode_js_composer_frontend_not_allowed_shortcodes', array() );
		...
		$output = sprintf( '<div class="vc_element" data-tag="%s" data-shortcode-controls="%s" data-model-id="%s">%s[%s %s]%s[/%s]%s'.$_div, esc_attr( $shortcode['tag'] ), esc_attr( wp_json_encode( $shortcode_obj->shortcodeClass()
		// END UNCODE EDIT
