...
window.vc || (window.vc = {}),
	...
	function($) {
		...
		_.isUndefined(window.vc) && (window.vc = {}), window.Vc_postSettingsEditor = Backbone.View.extend({
			...
			setSizeResizable: function() {
				...
				editorPositionTop = $editor.offset().top, height = vc.active_panel.$el.find('[data-vc-ui-element="panel-footer"]').offset().top - editorPositionTop - 27, this.aceEnabled() ? $editor.css({
					height: height,
					minHeight: height
				}) : (this.$editor.parent().css({
					height: height,
	...
	function(vc, _, $) {
		...
		window.vc_toTitleCase = function(str) {
			...
			return "vc_col-sm-", numbers = width ? width.split("/") : [1, 1], range = _.range(1, 13), num = !_.isUndefined(numbers[0]) && 0 <= _.indexOf(range, parseInt(numbers[0], 10)) && parseInt(numbers[0], 10), dev = !_.isUndefined(numbers[1]) && 0 <= _.indexOf(range, parseInt(numbers[1], 10)) && parseInt(numbers[1], 10), !1 !== num && !1 !== dev ? "vc_col-sm-" + 12 * num / dev : "vc_col-sm-12"
		...
		function processImages(attachments, callback) {
			...
			$.ajax({
				...
				if ("function" == typeof callback) {
					for (models = [], i = 0; i < response.data.ids.length; i++)(attachment = "function" == typeof attachment ? attachments.get(response.data.ids[i]) : attachments[response.data.ids[i]]) || (attachment = media.model.Attachment.get(response.data.ids[i])), models.push(attachment);
					var promises = function(models, callback) {
	...
	function($) {
		...
		VcSortedList.prototype = {
			...
			init: function() {
				...
				for (var i in selected_data) {
					...
					!1 !== ($control = !!(control_settings = selected_data[i].split("|")).length && this.$toolbar.find("[data-element=" + decodeURIComponent(control_settings[0]) + "]")) && $control.is("input") && ($control.prop("checked", !0), this.createControl({
						value: $control.val(),
		...
		vc.edit_form_callbacks = [], vc.atts = {
			...
			parse: function(param) {
				...
				return $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"), thumbnails_html = "", $field.parent().find("li.added").each(function() {
			...
			parse: function(param) {
				var $field, image_src;
				return image_src = "", ($field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]")).parent().find("li.added").length && (image_src = $field.parent().find("li.added img").attr("src")), $("[data-model-id=" + this.model.id + "]").data("field-" + param.param_name + "-attach-image", image_src), $field.length ? $field.val() : null
			},
			render: function(param, value) {
		...
		void 0 === window.vc && (window.vc = {}), window.vc.ShortcodesBuilder = function(models) {
			...
			save: function(status) {
				...
				string = this.getContent(), (data = {
                    ...
                    window.vc.unsetDataChanged(), window.vc.showMessage(window.i18nLocale.vc_successfully_updated || "Successfully updated!", 'save')
				})
			},
		...
		_.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function() {
			...
			createElement: function(e) {
				...
				if (!this.preventDoubleExecution) {
					...
					this.preventDoubleExecution = !0, e && e.preventDefault && e.preventDefault(), this.do_render = !0;
			...
			show: function() {
				if (!this.$el.hasClass("vc_active")) {
					// START UNCODE EDIT
					// vc.closeActivePanel(l), this.init(), (vc.active_panel = this).clicked = !1, this.$el.removeClass("vc_panel-opacity");
					vc.closeActivePanel(this.$el), this.init(), (vc.active_panel = this).clicked = !1, this.$el.removeClass("vc_panel-opacity");
					// END UNCODE EDIT
	...
	function($) {
		...
		window.vc.element_start_index = 0, window.vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
			...
			createElement: function(e) {
				...
				else "vc_row" === tag ? ("vc_section" === this.model.get("shortcode") ? (window.vc.storage.lock(), row = vc.shortcodes.create({
					shortcode: "vc_row",
					params: row_params,
					parent_id: this.model.id,
					order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.getNextOrder()
			...
			getFirstPositionIndex: function() {
				return window.vc.element_start_index -= 1, vc.element_start_index
			},
			show: function() {
	...
	function($) {
		...
		}, vc.showMessage = function(message, type) {
			if ( type === 'save' ) {
		...
		window.vc.events.on("shortcodeView:updated", function(model) {
			...
			prependElement: function(e) {
				...
				if ( this.model.attributes.shortcode === 'uncode_slider' ) {
					...
					vc.builder.create({
						shortcode: "vc_column_inner",
						params: { width: "1/1", el_class: "added-owl-item" },
	...
	function($) {
		...
		window.InlineShortcodeViewContainerWithParent = window.InlineShortcodeViewContainer.extend({
			...
			switchControls: function(e) {
				var $parent, $current;
				e && e.preventDefault && e.preventDefault(), vc.unsetHoldActive(), ($parent = $(e.currentTarget).parent()).addClass("vc_active"), ($current = $parent.siblings(".vc_active")).removeClass("vc_active"), $current.hasClass("vc_element") || window.setTimeout(this.holdActive, 500)
			}
		})
	}(window.jQuery),
	function() {
	...
	function($) {
		...
		window.InlineShortcodeView_vc_row = window.InlineShortcodeView.extend({
			...
			render: function() {
				var $content = this.content();
				return $content && $content.hasClass("vc_row-has-fill") && ($content.removeClass("vc_row-has-fill"), this.$el.addClass("vc_row-has-fill")), window.InlineShortcodeView_vc_row.__super__.render.call(this), this.manipulate(), this
			},
	...
	function($) {
		...
		window.InlineShortcodeView_vc_column = window.InlineShortcodeViewContainerWithParent.extend({
			...
			render: function() {
				return window.InlineShortcodeView_vc_column.__super__.render.call(this), this.prepend = !1, $('<div class="vc_resize-bar"></div>').appendTo(this.$el).mousedown(this.startChangeSize), this.setColumnClasses(), this.customCssClassReplace(),  vc.events.trigger("vc_column:render", this.$el), this
			},
			destroy: function(e) {
	...
	function($) {
		...
		window.vc.cloneMethod_vc_tab = function(data, model) {
			...
			changed: function() {
				this.allowAddControlOnEmpty() && 0 === this.$el.find(".vc_element[data-tag]").length ? this.$el.addClass("vc_empty").find("> :first > div:not(.vc_controls), > :first + div > div:not(.vc_controls)").addClass("vc_empty-element") : this.$el.removeClass("vc_empty").find("> :first > div:not(.vc_controls), > :first + div > div").removeClass("vc_empty-element"), this.setSorting()
			},
			...
			beforeUpdate: function() {
				// START UNCODE EDIT
				this.$el.parents( '.vc_element-container' ).eq(0).data( 'single-tabsNav', this.$tabsNav.html() );
				var $navTabs = this.$el.parents( '.vc_element-container' ).eq(0).data( 'single-tabsNav' );
				// END UNCODE EDIT
			...
			updated: function() {
				...
				// START UNCODE EDIT
				var $navTabs = this.$el.parents( '.vc_element-container' ).eq(0).data( 'single-tabsNav' );
				this.$tabsNav.html( $navTabs );
				if ( this.model.changed.params.vertical === 'yes' ) {
			...
			addTab: function(model) {
				...
				var $control = this.buildControlHtml(model),
					$content = this.tabsControls().find('.tab-content');
				// START UNCODE EDIT
				...
				return model.get("cloned") && this.tabsControls().find("[data-tab-id=tab-" + model.get("cloned_from").getParam("tab_id") + "]").length ? model.get("cloned_appended") || ($control.appendTo(this.tabsControls()), model.set("cloned_appended", !0)) : $control.appendTo(this.tabsControls()), this.changed(), !0
				// END UNCODE EDIT
			},
			...
			updateIfExistTab: function(model) {
				...
				var $tab = this.tabsControls().find("[data-tab-id=tab-" + model.getParam("tab_id") + "]");
				return !!$tab.length && ($tab.attr("aria-controls", "tab-" + model.getParam("tab_id")).find("a > span").attr("href", "#tab-" + model.getParam("tab_id")).text(model.getParam("title"), $tab.data("model", model).attr('data-m-id', model.get("id"))), !0)
				// END UNCODE EDIT
			},
			buildControlHtml: function(model) {
				...
				var $tab = $('<li data-tab-id="tab-' + model.getParam("tab_id") + '" class="vc_tta-tab' + active + '"><a href="#tab-' + model.getParam("tab_id") + '" data-toggle="tab"><span></span></a></li>');
				return $tab.data("model", model).attr('data-m-id', model.get("id")), $tab.find("> a > span").text(model.getParam("title")), $tab
				// END UNCODE EDIT
			},
			addElement: function(e) {
	...
	function($) {
		...
		window.InlineShortcodeView_vc_accordion = window.InlineShortcodeView_vc_row.extend({
			...
			changed: function() {
				this.allowAddControlOnEmpty() && 0 === this.$el.find(".vc_element[data-tag]").length ? this.$el.addClass("vc_empty").find("> :first:not(.vc_controls), > :first + div:not(.vc_controls)").addClass("vc_empty-element") : (this.allowAddControlOnEmpty() && this.$el.removeClass("vc_empty").find("> .vc_empty-element").removeClass("vc_empty-element"), this.setSorting())
			},
	...
	function($) {
		...
		vc.createPreLoader = function() {
			...
			if (!vc.loaded) {
				...
				}), vc.$frame.get(0).contentWindow.vc_js && vc.$frame.get(0).contentWindow.vc_js(), $(window).trigger("vc_build")
			}
		...
		$(function() {
			...
			window.listenKeyboardEvents = function(e, w, is_tinymce) {
				...
				if (e[w.uncodeFrontEditorShortkeysConf.keys.modifier] && e.which === parseInt(w.uncodeFrontEditorShortkeysConf.keys.save, 10)) {
					...
					if (w.$('#vc_ui-panel-edit-element.vc_active [data-vc-ui-element="button-save"]').length) {
						// Save module
						w.$('#vc_ui-panel-edit-element.vc_active [data-vc-ui-element="button-save"]').click();
					} else if (w.$('#vc_ui-panel-post-settings.vc_active [data-vc-ui-element="button-save"]').length) {
						// Save post settings (custom CSS)
						w.$('#vc_ui-panel-post-settings.vc_active [data-vc-ui-element="button-save"]').click();
					} else {
						// Save page
						w.vc.builder.save();
					}
					...
					if ( window.vc.active_panel ) {
						$('[data-vc-ui-element="button-close"]', window.vc.active_panel.$el).click();
						if ( $('body').hasClass('vc-sidebar-switch') ) {
							window.vc.add_element_block_view.render(!1);
						}
					} else if (w.$('#vc_ui-panel-edit-element.vc_active [data-vc-ui-element="button-close"]').length) {
						w.$('#vc_ui-panel-edit-element.vc_active [data-vc-ui-element="button-close"]').click();
					}
					...
					if (w.$('#vc_ui-panel-edit-element.vc_active .vc_ui-tabs-line').length) {
						var tabs = window.$('#vc_ui-panel-edit-element.vc_active .vc_ui-tabs-line');
						var current_tab = w.$('#vc_ui-panel-edit-element.vc_active .vc_ui-tabs-line').find('li.vc_active');
						...
						var target_tab = w.$('#vc_ui-panel-edit-element.vc_active .vc_ui-tabs-line').find('li[data-tab-index="' + (target_tab_index) + '"]');
