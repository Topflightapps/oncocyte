...
function uncode_page_options()
{
	...
	*/
	...
	$menu_no_shadow = array(
		...
		'type' => 'on-off',
		'desc' => esc_html__('Override to remove the shadow eventually declared in \'Menu -> Visuals\'.', 'uncode-core') ,
		'std' => 'off',
	);
	...
	$header_type = array(
		...
		'desc' => esc_html__('Override the Header type.', 'uncode-core') ,
		'std' => '',
		'type' => 'select',
		'choices' => array(
			array(
	...
	$body_fields = array(
		...
		run_array_mb($specific_featured_media, '_uncode_specific_media:not(on)') ,
	...
	if ($post_type === 'product') {
		...
		$body_fields[] = run_array_mb($specific_enable_sticky_desc, '_uncode_product_image_layout:is(),_uncode_product_image_layout:is(stack)');
		$body_fields[] = run_array_mb($specific_enable_woo_zoom);
		$body_fields[] = run_array_mb($specific_thumb_cols, '_uncode_product_image_layout:is(),_uncode_product_image_layout:is(std)');
		$body_fields[] = run_array_mb($specific_enable_woo_slider, '_uncode_product_image_layout:is(),_uncode_product_image_layout:is(std)');
	}
